...
| `missing_reason` | `correct --reason` blank or whitespace-only — not in the original catalogue; added so `decide` never writes something `CorrectionSchema`'s `min_length=1` would then refuse to read back (gate-soundness, same spirit as `missing_endpoint`) |
...
**Phase 4b — shipped.** `sumac add`/`decide.decide_change` and `sumac snapshot` both construct v2 events (`sumac.events`) directly and serialize them via `decide.serialize_event` (public — the snapshot command uses it too, not just `decide_change`). `SCHEMA_VERSION` bumped to 2. `schemas.py`'s `RecordSchema` now routes `(schema_version, type)` to the right v1 or v2 payload schema via a `mode="before"` validator, not just structural union matching — needed because a *zero-entry* v1 `InventorySnapshotSchema` and v2 `SnapshotSchemaV2` are structurally identical (both just `location_id` + an empty `entries` tuple), so with nothing to disambiguate on, pydantic's union resolution always picked whichever was listed first regardless of `schema_version`. Found by a test for exactly the empty-snapshot case §3.3a's whole design is about — would have silently misread a v2 empty snapshot as v1 (or vice versa) otherwise.

§3.5's insufficient-stock behavior (deferred twice, once each in Phase 3 and 4a) is implemented: a delta event whose `frm` side would be recorded below zero gets a `Counted` correction emitted first, informational note printed, no `--force`. Its "then the movement" ordering guarantee turned out not to be free: sharing `occurred_at` between the two writes made the fold's `(ts, actor, id)` sort tie-break on a random `uuid4`, so the movement committed before the correction close to half the time — found by manually smoke-testing the feature end to end (a wrong final quantity), not by any unit test, since `decide_change`'s own tests only ever checked the *returned* write order, never storage-and-refold. Fixed by giving the `Counted` write a timestamp one microsecond earlier, and covered by a new regression test that goes through actual storage.

Also fixed a **latent gap from Phase 3**, found while rewriting event construction for v2 rather than by a failing test: `InventoryChange.__post_init__` only ever checked that a *required* endpoint was present, never that an unexpected one was *absent* — `sumac add purchase milk 1 l --from pantry --to fridge` would have silently stored the spurious `from_location`, and the fold would have subtracted from it. `events.py`'s dataclasses have no `__post_init__` of their own, so this needed an explicit check (`_check_endpoint_shape`) rather than continuing to get it for free.
*Acceptance, as shipped:* round-trip write-then-fold produces expected state for every new event type (`tests/test_decide.py`, `tests/test_ledger.py`) — including a dedicated mixed-log test (a v1 record and a v2 record in the same log folding to the correct combined total) and an empty-v2-snapshot-resets-a-location test, both directly targeting the two riskiest cases §3.3a identified. Re-verified read-only against the real log (all still v1): identical to before this phase in every observable respect.

**Phase 5 — shipped.** `events.Correction(reason)` added to the v2 `Event` union (cancel-only, no `actor` — `Record.actor` already carries it), with `CorrectionSchema` (`min_length=1` on `reason`) and a no-op fold case. `ledger.load_all_records` added alongside `load_records`: the write-time guard needs the *unfiltered* view (live and already-superseded records both) to tell `supersede_target_missing` apart from `supersede_already_applied` — `load_records`'s own supersedes-filtering already erases that distinction. `decide.decide_correct` adds a `missing_reason` rejection not in the original catalogue, for the same gate-soundness reason `missing_endpoint` was added in Phase 3: `decide` must never write something `CorrectionSchema`'s own `min_length=1` would then refuse to read back. `sumac correct <record-id> --reason ...` appends the correction to the *current user's own* log segment regardless of whose record it targets, per §3.6's ownership rule. `sumac doctor` prints a ready-to-paste `sumac correct` command under the anomaly table for every anomaly with a `record_id`.
...
**Phase 7 — shipped.** Brought forward ahead of ship, rather than left optional: both fields are wire-permanent once written, and the household was about to start writing daily, so this was cheaper now than after.

`Record`/`RecordSchema` gained `seq: int | None = None` and `cmd_id: str | None = None` — both absent forever on config records (config is latest-revision-wins, not an append-sequential segment) and on everything written before this phase, which is the majority of the real log; there's no retroactive backfill onto existing lines, per "never modify existing log lines."

`seq` is assigned by `store.append`, not `decide` — it depends on what's already on disk, which `decide` (pure, no I/O) can't see. Before every log-stream append, `store.append` now reads the target segment (`store.verify_stream`, already tolerant), computes each existing record's `seq` via the new `store.assigned_seqs` (its own stored value if present, else its position among decoded records — the same backfill the v1 upcaster already relies on, justified the same way: these files are append-only and have never been reordered), and assigns `max + 1`. `sumac doctor` runs the same `assigned_seqs` per segment and flags `seq_duplicate` (the same seq value twice — a bad merge) and `seq_gap` (checked against the range starting at 0, not the lowest seq present, so truncation from the very start of a segment is caught too, not just the middle).

`cmd_id` is assigned by `decide`/`cli` — a fresh uuid4 per logical command (not per record), generated once and threaded through every record that command produces (e.g. §3.5's Counted plus the event it precedes share one). §3.7's "deduplicated during the fold" turned out to need a real decision, not just wiring: the two records a single command can produce are never duplicates of each other (they're deliberately different content sharing one causal origin), so cmd_id can't be *the* dedup key on its own — deduping by "cmd_id already seen" would silently drop the second, legitimate record of a multi-record command. What "protects against a pull ever replaying an event" concretely needs is deduping the exact same record showing up twice, and record `id` (a uuid4, unique by construction) already does that unambiguously — a literal duplicate line replayed by a bad merge decrypts to identical plaintext, same `id` both times. Shipped: fold-time dedup by `id` (new `duplicate_record` anomaly, loud not silent per §3.2 — the fold recovers on its own, but a human should still see it happened), in `ledger._fold`. `cmd_id` is stored on every new record for whatever a real sync protocol needs later, but isn't yet the dedup key itself — there's no sync mechanism in the codebase to define "replay with regenerated ids" against, and guessing that semantics wrong on a permanent wire format two people now depend on was a worse trade than shipping the narrower, unambiguous thing that already covers the concrete threat.

*Acceptance, as shipped:* `store.append` assigns monotone-per-stream seq starting at 0, ignores/overwrites any caller-supplied `seq`, and never assigns one to config. `sumac doctor` correctly detects both anomaly types from simulated bad-merge (duplicated line) and truncation (removed line) scenarios, and the fold applies a duplicated record's effect exactly once. Doctor's ready-to-paste suggestions dedupe by record id (a single record can trip more than one anomaly type — e.g. a duplicated line is both `seq_duplicate` and `duplicate_record` — and a second `sumac correct` of an already-corrected target would just fail). Re-verified against the real log: 16 anomalies (unchanged from Phase 5's post-cleanup count), no new `seq_gap`/`seq_duplicate`/`duplicate_record` findings — the real household log has no structural bad-merge or truncation history.
