...
- `AgentRunner.__init__(data_dir, key, *, runner=None)` builds a real `mistralrs.Runner` from `Which.GGUF(quantized_model_id="unsloth/Qwen3-1.7B-GGUF", quantized_filename="Qwen3-1.7B-Q4_K_M.gguf")` with no `tool_callbacks` argument, or accepts an injected `runner` satisfying `SendsCompletions` (llm.py:95-96, llm.py:329-346, llm.py:356-378).
- `AgentRunner.propose(prompt)` and `.revise(feedback)` drive a client-side tool-calling loop against `self._messages`, a plain Python list the instance owns — `_run_loop` dispatches `response.choices[0].message.tool_calls[0]` against `self.tool_callbacks` directly and appends the exchange as new messages, rather than registering `tool_callbacks` on `Runner` (llm.py:488-527, llm.py:549-563; see §25 for why).
...
- `sumac_find_inventory`'s callback filters `ledger.build_inventory` the same way `render.print_find` does — case-insensitive substring match on `product_id` — and returns `location_path` via `config.location_path` (llm.py:382-398).
...
- `src/sumac/cli.py`'s `ask` command takes `prompt: str`, `--dry-run`, and `--data-dir` (cli.py:373-380); its loop calls `render.print_trace(plan.trace)` on every iteration before either `render.print_plan(plan)` (writes present) or printing `plan.reply_text` (no writes), and prompts for `accept`/`reject`/free-text feedback unless `--dry-run` is set (cli.py:413-440).
...
- `tests/test_llm.py` (426 lines) drives `AgentRunner` against a hand-built `FakeRunner` satisfying `SendsCompletions`; one `ScriptedResponse` is one `send_chat_completion_request` round (a single tool call or a final reply), not a whole multi-call turn, matching the client-side loop's actual call granularity (tests/test_llm.py, full file).
- `tests/test_cli.py` has six `test_ask_*` functions (read-only reply, tool-call trace display, `--dry-run`, accept, reject, feedback-then-accept) driving the `ask` command's interactive loop against a scripted `_FakeAgentRunner` monkeypatched onto `sumac.llm.AgentRunner` (tests/test_cli.py:492-644).
- `pytest` reports 223 passing tests repo-wide; `ruff format --check .`, `ruff check .`, and `ty check` each report no findings (commands run from the repository root).
- `sumac ask "where is the jam?" --dry-run`, run against a real vault and the configured model on the user's machine, completes without the tool-registration error §25 records and asks a disambiguating question naming two "jam"-matching products in the vault (Scotch Bonnet Jam, Jamaican Style Hot Sauce) rather than guessing between them.
...
- No automated test exercises `AgentRunner` against a real `mistralrs.Runner` — `tests/test_llm.py`'s `FakeRunner` and `tests/test_cli.py`'s `_FakeAgentRunner` are both hand-built stand-ins (§22's intended split); the two manual runs recorded under Current State are the only real-model exercise of this code, and neither is captured as a test.
- No compound multi-write request (the ragu/tomatoes reference example, §2) or any write-producing request (`consume`/`move`/`discover`) has been run against a real model — both manual runs recorded above are read-only (`find` only).
- `SYSTEM_PROMPT` (llm.py:113-266) is unchanged from §20's draft text; it has not been tuned against the real model beyond the two read-only requests above.
- `QUANTIZED_MODEL_ID`/`QUANTIZED_FILENAME` (llm.py:95-96) are the values used for the two real runs recorded above; no comparison against another GGUF model or quantization has been made, and no run has exercised more than a single-turn read-only request against them.
- No flag or setting suppresses `render.print_trace`'s output — the tool-call trace prints unconditionally whenever `plan.trace` is non-empty (cli.py:414, render.py:289-303).
