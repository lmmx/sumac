...
def agent_runner_factory(request: pytest.FixtureRequest, inventory: tuple[Path, bytes]):
    ...
    reload/reconnect). Skips — never errors, never attempts a network
    download — when the GGUF isn't already in the local cache; see
    `llm.is_cached`."""
...
def _build_eval_payload(
    *,
    ...
    `tests/test_eval_conftest.py`. `"backend"` is hardcoded to `"local"` —
    kept as a constant rather than dropped, so old and new `--eval-json`
    output share one schema; see `epoch_report.py`'s `_group_label`."""
    rates = [r.tokens_per_sec for r in results if r.tokens_per_sec is not None]
    return {
        "model": model_name,
